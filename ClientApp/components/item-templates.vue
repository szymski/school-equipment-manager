    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th style="width:1px;">Nazwa</th>
                <th>Opis dodatkowy</th>
                <th></th>
            </tr>
        </thead>
        <tbody>
            <!-- <tr v-for="itemTemplate in filterItems(items)" v-bind:key="item.id">
                <td>0</td>
                <td>{{ item.id }}</td>
                <td>{{ item.name }}</td>
...
export default {
    ...
    methods: {
        ...
        async removeItem(id) {
            await this.$http.post('/api/ItemsTemplates/Remove', "id=" + id);
            let response = await this.$http.get('/api/ItemsTemplates')
